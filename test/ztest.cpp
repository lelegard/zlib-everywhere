...
using Compress = void (*)(ByteBlock& out, const void* in, size_t in_size, int level);
using Decompress = void (*)(ByteBlock& out, const void* in, size_t in_size);

//----------------------------------------------------------------------------
// Testing with headers-only implementation from https://github.com/vurtun/lib
//----------------------------------------------------------------------------
...
#include "../vurtun-lib/sdefl.h"
#include "../vurtun-lib/sinfl.h"
...
void dump(const char* margin, const uint8_t* data, size_t data_size, size_t line_size)
{
    ...
    while (data < end) {
        ...
        const char* space = "";
        while (len-- > 0) {
            std::cout << space << "0x" << hexa(*data >> 4) << hexa(*data & 0x0F) << ",";
            data++;
            space = " ";
        }
        std::cout << std::endl;
...
void compress_with_zlib(ByteBlock& out, const void* in, size_t in_size, int level)
{
    ...
    checkZlib(strm, status, "deflateEnd");
}
...
void decompress_with_zlib(ByteBlock& out, const void* in, size_t in_size)
{
    ...
    checkZlib(strm, status, "inflateEnd");
}
...
void compress_with_sdefl(ByteBlock& out, const void* in, size_t in_size, int level)
{
    ...
    if (len < 0) {
        std::cerr << "*** sdefl error " << len << " from zsdeflate" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    else {
...
void decompress_with_sdefl(ByteBlock& out, const void* in, size_t in_size)
{
    ...
    if (len < 0) {
        std::cerr << "*** sdefl error " << len << " from zsinflate" << std::endl;
        std::exit(EXIT_FAILURE);
    }
    else {
        out.resize(size_t(len));
...
const std::vector<Library> libraries {
    {"sdefl", compress_with_sdefl, decompress_with_sdefl},
#if defined(USE_ZLIB)
    {"zlib", compress_with_zlib, decompress_with_zlib},
#endif
};
...
void decompress_verify(const ByteBlock& compressed, Decompress decomp)
{
    ...
    if (out.size() != intext_size || ::memcmp(out.data(), intext, intext_size) != 0) {
        std::cout << "*** invalid decompressed data";
        if (out.size() != intext_size) {
            std::cout << ", decompressed size: " << out.size() << ", reference size: " << intext_size;
        }
        size_t diff = 0;
        while (diff < out.size() && diff < intext_size && out[diff] == uint8_t(intext[diff])) {
            diff++;
        }
        std::cout << ", first diff at index " << diff << std::endl;
    }
}
...
{
#if defined(USE_ZLIB)
    std::cout << "zlib version: " << zlibVersion() << " (compiled with " << ZLIB_VERSION << ")" << std::endl;
#endif

    // With -g, generate reference data.
    if (argc >= 2 && std::string(argv[1]) == "-g") {
        generate_reference(intext, intext_size, 1);
        generate_reference(intext, intext_size, 4);
        generate_reference(intext, intext_size, 9);
        return EXIT_SUCCESS;
    }
    ...
    for (const auto& lib : libraries) {
        for (const auto& ref : references) {
            std::cout << "--- Decompress reference with " << lib.name << ", level " << ref.level << std::endl;
            decompress_verify(ref.data, lib.decompress);
        }
    }

    // Compress and decompress, all combinations.
    for (const auto& lib_comp : libraries) {
        for (const auto& lib_decomp : libraries) {
            std::cout << "--- Compress with " << lib_comp.name << ", decompress with " << lib_decomp.name << std::endl;
            for (int level = 0; level <= 9; level++) {
                ByteBlock compressed;
                lib_comp.compress(compressed, intext, intext_size, level);
                std::cout << "    Level " << level << ", compressed size: " << compressed.size()
                          << " (original: " << intext_size << ")" << std::endl;
                decompress_verify(compressed, lib_decomp.decompress);
            }
        }
    }
